...
Please summarize the feedback, identify the majority opinion, restate the original question, and provide the final correct answer."""
    ...
    def test_extract_xml_identity(self):
        ...
        self.assertEqual(extract_xml_identity("<majority>student2</majority>"), "student2")
        self.assertEqual(extract_xml_identity("Some text <majority>student3</majority> more text"), "student3")
        # Edge cases
    ...
    def test_extract_xml_final_answer(self):
        ...
        self.assertEqual(extract_xml_final_answer("Some text <answer>456</answer> more text"), "456")
        # Edge cases
    ...
    def test_extract_xml_question(self):
        ...
        self.assertEqual(extract_xml_question(self.sample_text_good), "What is 6 times 7?")
        self.assertEqual(extract_xml_question("<question>What is 2+2?</question>"), "What is 2+2?")
        self.assertEqual(extract_xml_question("Some text <question>What is 3*3?</question> more text"), "What is 3*3?")
        # Edge cases
    ...
    def test_consensus_reward_func(self):
        ...
        with patch('hivemind_exp.gsm8k.stage3_rewards.extract_xml_choices') as mock_choices, \
             patch('os.makedirs', return_value=None), \
             patch('builtins.open', return_value=MagicMock()):
            
            # Set up mocks
            mock_choices.return_value = ["student1", "student1", "student2"]  # student1 is the majority
            
            # Test with student1 (matches majority)
            rewards1 = consensus_reward_func(
                self.mock_prompts, [self.mock_completions[0]], weighting=2.0, logging=False
            )
    ...
    def test_question_recreation_reward_func(self):
        ...
        with patch('hivemind_exp.gsm8k.stage3_rewards.extract_original_question') as mock_original, \
             patch('os.makedirs', return_value=None), \
             patch('builtins.open', return_value=MagicMock()):
            ...
            rewards1 = question_recreation_reward_func(
                self.mock_prompts, [self.mock_completions[0]], weighting=1.0, logging=False
            )
            
            # Test with slightly different question
            different_question = [[{"content": "<question>\nWhat is six times seven?\n</question>\n"}]]
    ...
    def test_concensus_correctness_reward_func(self):
        ...
        with patch('hivemind_exp.gsm8k.stage3_rewards.extract_answers') as mock_answers, \
             patch('hivemind_exp.gsm8k.stage1_rewards.extract_xml_answer') as mock_extract, \
            ...
            mock_extract.side_effect = lambda x: "42" if "student1" in x else "43" if "student2" in x else "13"
            ...
            rewards1 = concensus_correctness_reward_func(
                self.mock_prompts, student1, self.mock_answer, weighting=2.0, logging=False
            )
            ...
            rewards_none_correct = concensus_correctness_reward_func(
                self.mock_prompts, none_student, self.mock_answer, weighting=2.0, logging=False
            )
    ...
    def test_final_correctness_reward_func(self):
        ...
        with patch('os.makedirs', return_value=None), \
            ...
            rewards1 = final_correctness_reward_func(
                self.mock_prompts, [self.mock_completions[0]], self.mock_answer, weighting=2.0, logging=False
            )
            
            # Test with incorrect answer
            wrong_answer = [[{"content": "<answer>\n43\n</answer>\n"}]]
            rewards2 = final_correctness_reward_func(
                self.mock_prompts, wrong_answer, self.mock_answer, weighting=2.0, logging=False
            )
            
            # Test with non-numeric answer
            non_numeric = [[{"content": "<answer>\nforty-two\n</answer>\n"}]]
            rewards3 = final_correctness_reward_func(
                self.mock_prompts, non_numeric, self.mock_answer, weighting=2.0, logging=False
            )
    ...
    def test_strict_format_reward_func(self):
        ...
        with patch('os.makedirs', return_value=None), \
            ...
            rewards = strict_format_reward_func(completions, weighting=0.5, logging=False)
    ...
    def test_hivemind_cumulative_reward(self):
        ...
        with patch('hivemind_exp.gsm8k.stage3_rewards.consensus_reward_func') as mock_consensus, \
             patch('hivemind_exp.gsm8k.stage3_rewards.concensus_correctness_reward_func') as mock_correctness, \
             patch('hivemind_exp.gsm8k.stage3_rewards.question_recreation_reward_func') as mock_question, \
             patch('hivemind_exp.gsm8k.stage3_rewards.final_correctness_reward_func') as mock_final, \
             patch('hivemind_exp.gsm8k.stage3_rewards.strict_format_reward_func') as mock_strict, \
             patch('hivemind_exp.gsm8k.stage3_rewards.soft_format_reward_func') as mock_soft, \
             patch('hivemind_exp.gsm8k.stage3_rewards.xmlcount_reward_func') as mock_xml, \
             patch('hivemind_exp.gsm8k.stage3_rewards.extract_original_question') as mock_extract:
            ...
            rewards = hivemind_cumulative_reward(
                mock_node, self.mock_prompts, self.mock_completions, 
                self.mock_answer, output_signal_selector="max", logging=False
            )
            ...
            self.assertEqual(mock_node.outputs["final_agent_decision"][mock_node.key], self.sample_text_good)
            ...
            rewards = hivemind_cumulative_reward(
                mock_node, self.mock_prompts, self.mock_completions, 
                self.mock_answer, output_signal_selector=None, logging=False
            )
            ...
            rewards_empty = hivemind_cumulative_reward(
                mock_node, self.mock_prompts, self.mock_completions, 
                [], output_signal_selector="max", logging=False
            )
